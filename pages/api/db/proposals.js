  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      const deal = await prisma.deal.create({
        data: { leadId, value, currency: "USD", stage: "PROPOSAL", probability: 60, status: "open", notes: notes || "" },
      });

      await Promise.all([
        prisma.leadEvent.create({
          data: { leadId, eventType: "PROPOSAL_SENT", actorType: "HUMAN", title: `Proposal sent — $${value}`, metadata: { dealId: deal.id, value } },
        }),
        prisma.lead.update({ where: { id: leadId }, data: { status: "interested" } }),
      ]);
      ...
      const emailRes = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          html: `<!DOCTYPE html><html><head><meta charset="utf-8"><style>
            ...
            <body>
              <div class="header"><h1>ProSites</h1><p>Website Proposal for ${lead.company}</p></div>
              <div class="body">
                ...
                <div class="section"><h3>What's included</h3>${proposalScope}</div>
                <div class="section">
                ...
                <a href="mailto:vishal0786sandhu@gmail.com?subject=Re: Website Proposal for ${lead.company}" class="cta">✅ Accept Proposal — Reply to this email</a>
              ...
              <div class="footer">Vishal · outreach@pro-sites.in<br/><a href="mailto:vishal0786sandhu@gmail.com?subject=Unsubscribe">Unsubscribe</a></div>
            </body></html>`,
        }),
      });
      ...
      if (SLACK) {
        await fetch(SLACK, {
          ...
          body: JSON.stringify({ text: `💼 *Proposal Sent!*\n👤 ${lead.firstName} — ${lead.company}\n💰 $${value}\n📧 ${lead.email}\n\n🔥 Follow up in 24 hours!` }),
        });
      }

      return res.status(200).json({ success: true, deal, emailSent: emailRes.ok });
    }
    ...
    if (req.method === "PATCH") {
      ...
      const deal = await prisma.deal.update({
        where: { id },
        data: { ...(stage && { stage }), ...(status && { status }), ...(value && { value }) },
        include: { lead: true },
      });

      if (status === "won") {
        await Promise.all([
          prisma.lead.update({ where: { id: deal.leadId }, data: { status: "closed" } }),
          prisma.leadEvent.create({
            data: { leadId: deal.leadId, eventType: "DEAL_WON", actorType: "HUMAN", title: `Deal WON — $${deal.value}`, metadata: { dealId: id, value: deal.value } },
          }),
        ]);
        if (SLACK) {
          await fetch(SLACK, {
